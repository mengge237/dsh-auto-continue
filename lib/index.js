// dsh-auto-continue v0.4.2 — max-tokens 自动续写 + 429/限流自动恢复 + 压缩空转补枪
//
...
//
// v0.4.2（2026-09-25，0.1.7 / 会话格式 v4 适配，两条都是硬闸）：
//   1) 续写消息的 source 改为 {kind:'plugin:dsh-auto-continue'}（不再带 plugin 字段）。
//      v4 的原生写侧 assertV4MessageSources 对**任何** kind === 'plugin' 的消息源直接拒：
//      "format v4 message requires a producer-owned source kind"
//      （dsh-session-persistence-jsonl/lib/worker.cjs:10902/11683，0.1.7-rc.2 实测在装产物）。
//      本机 2026-09-25 18:54 实证：session-8c256625 turn 12 吃 429（RATE_LIMIT），插件按
//      61s 冷却补投重试消息，splice 在下一轮开头被 v4 闸打死——GUI 显示"本轮运行失败
//      format v4 message requires a producer-owned source kind"，而存档里什么都没留下
//      （被拒的是新事件，不是旧档；会话本身无损）。plugin:<name> 正是官方 v3→v4 迁移器
//      producerKind() 对未发布插件的改写形（dsh-session-format-v3-to-v4/lib/index.js:92），
//      与迁移后的历史行同 kind，不造假身份。
//   2) 压缩检查点认形状加收 {kind:'compact-checkpoint'}：0.1.7 的 dsh-compaction 把
//      COMPACT_CHECKPOINT_MARKER 定为该值（lib/index.js:109），且 v3→v4 迁移把历史行里的
//      plugin 'compact' 也改写成它（v3-to-v4 lib/index.js:52）。旧形状保留——0.1.5 时代存档
//      未迁移部分与形状回放测试都还是 {kind:'plugin',plugin:'compact'}。
//
// 环境变量：
...
/** 压缩检查点的消息归属：v4（0.1.7+）为 {kind:'compact-checkpoint'}，v3 时代形状是 {kind:'plugin',plugin:'compact'}。 */
const CHECKPOINT_KIND_V4 = 'compact-checkpoint'
...
/** 一轮里输出低于这个 token 数就不算「被截断的长答案」，算「输出预算被夹死」，别续写。 */
const CLAMP_TOKENS = num(process.env.DSH_AUTO_CONTINUE_CLAMP_TOKENS, 8, 1)
...
function makeMessage(text) {
  return Object.freeze({
    ...
    // v4 写侧要求 source.kind 为 producer 自有字符串，kind:'plugin' 会被硬拒（文件头 v0.4.2 注 1）。
    source: Object.freeze({ kind: 'plugin:' + name }),
  })
}
...
const isCompactCheckpoint = (e) => {
  ...
  if (s.kind === CHECKPOINT_KIND_V4) return true // 0.1.7 现写与 v3→v4 迁移后的存档
  return s.kind === 'plugin' && s.plugin === CHECKPOINT_PLUGIN // v3 及更早的存档形状
}
...
/**
 * 「输出预算被夹死」判定（本机 2026-09-22 存档实证，session-28de2f17 回合 12~18）：
 *   provider 只声明 id+name 时适配器退回内置默认 contextWindow 262144，pi-ai 的
 *   clampMaxTokensToContext 算出 available <= 0 就把 max_tokens 夹成 1 —— 模型只回
 *   一个 token 就 stopReason=length，落到存档正是 turn/end kind=max-tokens。
 *   这跟「长答案被截断」不是一回事：续写只会再夹一次，而每一发都要重带整份上下文
 *   （实测 6 发 × 430k 输入，5 分钟内把 TPM 打进 429 insufficient_quota）。
 * 只看这一轮最后那条 assistant 的 usage.outputTokens；读不到就交给老分支处理。
 */
function clampedOutputTokens(agent) {
  const evs = snapshotOf(agent)
  if (!evs) return null
  const start = Math.max(0, evs.length - SCAN_WINDOW)
  for (let i = evs.length - 1; i >= start; i -= 1) {
    ...
    const n = Number(e.data?.usage?.outputTokens)
...
export function apply(ctx) {
  ...
  try {
    ctx.on('agent/status', ({ agent, status }) => {
      ...
      if (kind === 'max-tokens' && !rateLimited) {
        const out = clampedOutputTokens(agent)
        if (out !== null && out <= CLAMP_TOKENS) {
          rec.handledTurn = end.turn // 一轮只喊一次，别每帧刷日志
          log('warn', agentId, 'max-tokens 但整轮只回 ' + out + ' 个 token（输出预算被夹到 ' + CLAMP_TOKENS + ' 以下，通常是 contextWindow 配得比端点真实窗口小）→ 不续写，换更大窗口的模型或先交接')
          return
        }
      }
      if (rateLimited) {
    ...
    log('info', '已挂载（v0.4.2）：max-tokens 自动续写 + 429 自动恢复 + 压缩空转补枪；上限 ' + MAX_PER_MIN + '/min，429 冷却 ' + Math.round(COOLDOWN_MS / 1000) + 's×' + MAX_429_RETRIES + '，新鲜度 ' + Math.round(STALE_MS / 1000) + 's，压缩窗口 ' + Math.round(CP_WINDOW_MS / 1000) + 's，日志 ' + (LOG_FILE || 'off'))
  } catch (err) {
