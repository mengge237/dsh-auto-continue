...
export function apply(ctx) {
  ...
  try {
    ...
    log('info', '已挂载（v0.4.1）：max-tokens 自动续写 + 429 自动恢复 + 压缩空转补枪；上限 ' + MAX_PER_MIN + '/min，429 冷却 ' + Math.round(COOLDOWN_MS / 1000) + 's×' + MAX_429_RETRIES + '，新鲜度 ' + Math.round(STALE_MS / 1000) + 's，压缩窗口 ' + Math.round(CP_WINDOW_MS / 1000) + 's，日志 ' + (LOG_FILE || 'off'))
  } catch (err) {
